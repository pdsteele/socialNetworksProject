temp <- read.table("outdeg_twitter", header=FALSE, sep=" ")
x1 <- temp[,1]
y1 <- temp[,2]

temp <- read.table("outdeg_TCL", header=FALSE, sep=" ")
x2 <- temp[,1]
y2 <- temp[,2]

temp <- read.table("outdeg_FRDG", header=FALSE, sep=" ")
x3 <- temp[,1]
y3 <- temp[,2]

temp <- read.table("outdeg_FRDG_TCL", header=FALSE, sep=" ")
x4 <- temp[,1]
y4 <- temp[,2]
...
dev.copy(pdf,'generatorComparison_outDeg.pdf')
dev.off()