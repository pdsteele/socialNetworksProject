temp <- read.table("indeg_twitter", header=FALSE, sep=" ")
x1 <- temp[,1]
y1 <- temp[,2]

temp <- read.table("indeg_TCL", header=FALSE, sep=" ")
x2 <- temp[,1]
y2 <- temp[,2]

temp <- read.table("indeg_FRDG", header=FALSE, sep=" ")
x3 <- temp[,1]
y3 <- temp[,2]

temp <- read.table("indeg_FRDG_TCL", header=FALSE, sep=" ")
x4 <- temp[,1]
y4 <- temp[,2]
...
dev.copy(pdf,'generatorComparison_inDeg.pdf')
dev.off()